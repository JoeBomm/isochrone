...
// Mock the single matrix service
jest.mock('src/lib/algorithms/singleMatrix', () => ({
  singleMatrixService: {
    evaluateAllHypothesisPoints: jest.fn(),
...
const mockSingleMatrixService = singleMatrixService as jest.Mocked<
  typeof singleMatrixService
>
...
const locationArbitrary = fc.record({
  ...
  latitude: fc
    .float({ min: -80, max: 80, noNaN: true })
    .filter((lat) => Math.abs(lat) > 0.1), // Avoid poles and origin
  longitude: fc
    .float({ min: -170, max: 170, noNaN: true })
    .filter((lng) => Math.abs(lng) > 0.1), // Avoid antimeridian and origin
})

// Generator for arrays of unique locations (no duplicates)
const uniqueLocationsArbitrary = (minLength: number, maxLength: number) =>
  fc
    .array(locationArbitrary, { minLength, maxLength })
    .map((locations) => {
      ...
      const filtered = locations.filter((loc) => {
        ...
        for (const existingKey of seen) {
          ...
          const distance = Math.sqrt(
            Math.pow(roundedLat - existingLat, 2) +
              Math.pow(roundedLng - existingLng, 2)
          )
...
const travelModeArbitrary = fc.constantFrom(
  'DRIVING_CAR',
  'CYCLING_REGULAR',
  'FOOT_WALKING'
)
...
describe('Isochrone Service Property Tests', () => {
  beforeEach(() => {
    ...
    // Mock single matrix service
    mockSingleMatrixService.evaluateAllHypothesisPoints.mockResolvedValue({
      hypothesisPoints: [
        {
          id: 'test_point',
    ...
    mockSingleMatrixService.getApiCallCount.mockReturnValue(1)
    mockSingleMatrixService.resetApiCallCount.mockReturnValue(undefined)
    ...
    mockCachedOpenRouteClient.calculateTravelTimeMatrix.mockResolvedValue({
      ...
      destinations: [
        {
          id: '1',
          coordinate: { latitude: 40.7359, longitude: -73.9906 },
          phase: 'ANCHOR',
        ...
        {
          id: '5',
          coordinate: { latitude: 40.7439, longitude: -73.9928 },
          phase: 'ANCHOR',
          type: 'MEDIAN_COORDINATE',
        },
      ],
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc
            ...
            .filter(
              (s) =>
                !['DRIVING_CAR', 'CYCLING_REGULAR', 'FOOT_WALKING'].includes(s)
            ),
          fc.integer({ min: 5, max: 60 }),
